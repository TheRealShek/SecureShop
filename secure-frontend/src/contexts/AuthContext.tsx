  ...
  useEffect(() => {
        ...
        if (!session?.user) {
          ...
          if (cachedUser && cachedUser.id === session.user.id) {
            ...
            try {
              ...
              console.log(' Quick restore failed, doing full validation...');
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (shouldSkipOperation(2000)) {
        console.log(' Skipping auth change - recent tab visibility change');
        return;
      }
      
      // Don't process changes during initial load or certain background events
      if (!authReady && event !== 'SIGNED_OUT') {
        console.log(' Skipping auth change during initial load');
        return;
      }
      ...
      if (event === 'TOKEN_REFRESHED' && token && user && role && session?.access_token === token) {
        console.log(' Skipping token refresh - state already current');
        return;
      }
